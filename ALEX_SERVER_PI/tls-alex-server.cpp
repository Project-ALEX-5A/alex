...
#define CLIENT_NAME     "ShanTengMac"
...
void sendNetworkData(const char *data, int len)
{
  ...
  if (networkActive)
  {
    ...
    if (tls_conn != NULL) {
      ...
      bytes_written_to_tls = sslWrite (tls_conn, data, len);
      /* END TODO */
    }

    // Network is still active if we can write more then 0 bytes.
    networkActive = ( bytes_written_to_tls > 0 );
  }
}
...
void *worker(void *conn)
{
  ...
  while (networkActive)
  {
    ...
    // len is the actual number of bytes read 
